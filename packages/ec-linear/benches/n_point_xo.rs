...
mod n_point_xo {
    ...
    use ec_linear::recombinator::{
        n_point_xo::{
            NPointXoChunks, NPointXoMultiSwap, NPointXoPrimitive, NPointXoPrimitiveFloyds,
            NPointXoWindows,
        },
    ...
    #[divan::bench(consts = NUM_CROSSOVERS)]
    fn primitive_floyds<const N: usize>(bencher: Bencher) {
        bencher
            .with_inputs(|| {
                let first_parent = vec![0; GENOME_SIZE];
                let second_parent = vec![1; GENOME_SIZE];

                let crossover_operator = NPointXoPrimitiveFloyds::<N>::new().unwrap();

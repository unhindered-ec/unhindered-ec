...
/// An interface representing an evaluated member of the population.
///
/// Types implementing this trait must provide access to:
/// - A [`Genome`](Self::Genome) that defines the individual's encoded traits.
/// - The [`TestResults`](Self::TestResults) that score the individual's
///   performance, error, or quality.
///
...
/// ```
/// # use ec_core::{performance::score_value::ScoreValue, individual::Individual};
/// # use ec_linear::genome::bitstring::Bitstring;
/// #
/// # #[allow(dead_code)]
/// struct ScoredIndividual {
///     genome: Bitstring,
///     score: ScoreValue<u32>,
/// }
///
/// impl Individual for ScoredIndividual {
///     type Genome = Bitstring;
///     type TestResults = ScoreValue<u32>;
///
///     fn genome(&self) -> &Bitstring {
///         &self.genome
///     }
///
///     fn test_results(&self) -> &ScoreValue<u32> {
///         &self.score
///     }
/// }
/// ```
/// [^ec-linear-usage]: Note that this example uses the `Bitstring` type from
///     [`ec-linear`][ec-linear] which is not a
...
pub trait Individual {
    ...
    /// # }
    /// #
    /// let genome = [true, false, true].into_iter().collect::<Bitstring>();
    ///
    ...
    /// };
    ///
    /// assert_eq!(my_individual.genome(), &genome);
    /// ```
    /// [^ec-linear-usage]: Note that this example uses the `Bitstring` type from
    ///     [`ec-linear`][ec-linear] which is not a
    ///     dependency of this package to demonstrate some concepts which need

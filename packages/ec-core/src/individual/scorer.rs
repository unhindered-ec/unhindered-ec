...
///
/// impl Scorer<i32> for TenScorer {
///     type Score = u32;
///
///     fn score(&self, genome: &i32) -> Self::Score {
///         genome.abs_diff(10)
///     }
/// }
///
/// let score = TenScorer.score(&8);
/// assert_eq!(score, 2);
/// ```
pub trait Scorer<G> {
    type Score;
    ...
    T: Fn(&G) -> R,
{
    ...
    /// # Example
    /// ```
    /// # use ec_core::{performance::error_value::ErrorValue, individual::scorer::{Scorer, FnScorer}};
    /// #
    /// let scorer = FnScorer(|genome: &i32| ErrorValue(-*genome));
    ///

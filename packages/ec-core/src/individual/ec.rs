...
///
/// This might be generated by an [`IndividualDistribution`] wrapping a
/// genome generator (for the initial population).
///
/// The test results `T` are usually either a simple value (`i32`, etc.), a
/// [`ScoreValue`](ec_core::performance::score_value::ScoreValue) or
/// [`ErrorValue`](ec_core::performance::error_value::ErrorValue) or a
/// collection using a type like
/// [`TestResults`](ec_core::performance::test_results::TestResults).
///
/// # Example
/// In this example the genome is an `i32`. `scorer_1` computes the (absolute)
/// distance between an individual and the constant `11`, and `scorer_2` does
/// the same thing but with the constant `30`.
///
/// ```
...
impl<G, R> Individual for EcIndividual<G, R> {
    ...
    /// #
    /// let individual = EcIndividual::new(5, 0);
    /// #
    /// # // make type inference work.
    /// # let _: &EcIndividual<i32, i32> = &individual;
    ///
...
impl<G, R> From<(G, R)> for EcIndividual<G, R> {
    ...
    /// # use ec_core::individual::{Individual, ec::EcIndividual};
    /// #
    /// let individual = EcIndividual::from((80, 0));
    /// #
    /// # // make type inference work.
    /// # let _: &EcIndividual<i32, i32> = &individual;
    ///
...
impl<GD, S> IndividualDistribution<GD, S> {
    ...
    /// let individual_distribution = IndividualDistribution::new(genome_distribution, scorer);
    ///
    /// let my_individual: EcIndividual<i32, u32> = individual_distribution.sample(&mut rand::rng());
    /// #
    /// # assert!((i32::MIN..=i32::MAX).contains(my_individual.genome()));
    /// # assert!((u32::MIN..=u32::MAX).contains(my_individual.test_results()));
    /// ```
...
impl<GD> WithScorer for GD {
    /// Create a [`Distribution`] of [`EcIndividual`]s (i.e. scored genomes)
    /// from a distribution `GD` of genomes. The genomes are scored using  the

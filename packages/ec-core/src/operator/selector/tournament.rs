...
impl TournamentSizeError {
    ...
    ///
    /// # Safety
    /// The caller must ensure to uphold the invarant that `tournament_size >
    /// population_size`.
    #[must_use]
    pub const unsafe fn new(tournament_size: NonZeroUsize, population_size: usize) -> Self {
        debug_assert!(tournament_size.get() > population_size);
        Self {
            tournament_size,
            population_size,
        }
    ...
    P::Individual: Ord,
{
    ...
    fn select<'pop, R: Rng + ?Sized>(
        ...
        if population.size() < self.size.into() {
            // SAFETY: The if above checks the invariant tournament_size > population_size
            return Err(unsafe { TournamentSizeError::new(self.size, population.size()) });
        }
        population
            .as_ref()

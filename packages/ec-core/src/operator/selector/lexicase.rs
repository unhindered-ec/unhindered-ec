...
use super::{Selector, error::EmptyPopulation, random::Random};
    ...
    Res: Ord,
{
    ...
    fn select<'pop, R: Rng + ?Sized>(
        ...
        // use random selection on remaining candidates
        Random.select(&candidates, rng).copied().map_err(Into::into)
    }
}

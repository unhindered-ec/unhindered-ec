...
/// similarly to how one would use composable iterators over defining new
/// iterator types all the time.
///
/// # Example
/// In the example below:
///    - `Constant::new(false)` creates an operator which returns `false`
///      regardless of which value it is applied to.
///    - `.apply_n_times::<5>()` applies the previous operator (`Constant`) five
///      times to the input, generating an array of 5 instances of `false`.
///    - `.then_map(Identity)` maps the `Identity` operator over each of those
///      instances of `false`, returning another array of 5 instances of
///      `false`.
///    - `.then_map(Constant::new(10))` converts each of those `false` values
///      into `5`, yielding an array of 5 instance of the value 5.
///
/// These operations are all implemented in terms of the composable trait.
///
/// ```
...
///     .apply_n_times::<5>()
///     .then_map(Identity)
///     .then_map(Constant::new(10));
///
/// let Ok(my_result) = my_operator.apply((), &mut rng());
/// assert_eq!(my_result, [10; 5]);
/// ```
///
...
pub trait Composable {
    ...
    /// # use rand::rng;
    /// #
    /// let my_composed_operator = Constant::new(5).apply_n_times::<3>();
    /// let my_constructed_operator = RepeatWith::<_, 3>::new(Constant::new(5));
    ///
    /// assert_eq!(my_composed_operator, my_constructed_operator);
    ///
    /// let Ok(my_value) = my_composed_operator.apply((), &mut rng());
    /// assert_eq!(my_value, [5, 5, 5]);
    /// ```
    ///
    /// [`Operator`]: super::Operator
    ...
    ///     Constant::new(5).wrap::<GenomeScorer<_, _>>(FnScorer(|x: &i32| *x * 2));
    ///
    /// let Ok(my_individual) = my_individual_generator.apply(&[10], &mut rng());
    ///
    /// assert_eq!(my_individual, EcIndividual::new(5, 10));
    /// ```
...
/// #
/// let my_individual_generator =
///     GenomeScorer::construct(Constant::new(5), FnScorer(|x: &i32| *x + 7));
///
/// let Ok(my_individual) = my_individual_generator.apply(&[10], &mut rng());
///
/// assert_eq!(my_individual, EcIndividual::new(5, 12));
/// ```
///
/// [`Operator`]: super::Operator

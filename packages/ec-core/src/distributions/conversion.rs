...
/// assert!(my_collection.contains(&my_sample));
/// # let _ = my_sample;
/// # Ok(())
/// # }
/// ```
...
impl<T> IntoDistribution<T> for Vec<T>
where
    T: Clone,
{
    type Distribution = ChooseCloningOwning<Self, T>;
    type Error = Empty;
    ...
    /// assert!([0, 1, 2].contains(&my_value));
    /// # let _ = my_value;
    /// # Ok(())
    /// # }
    /// ```
...
impl<'a, T> IntoDistribution<&'a T> for &'a Vec<T> {
...
impl<'a, T> IntoDistribution<T> for &'a Vec<T>
where
    T: Clone,
{
...
impl<T, const N: usize> IntoDistribution<T> for [T; N]
where
    T: Clone,
{
    type Distribution = ChooseCloningOwning<Self, T>;
    ...
    ///
    /// let my_value: i32 = my_distr.sample(&mut rng());
    /// assert!(my_array.contains(&my_value));
    /// # let _ = my_value;
    /// # Ok(())
    /// # }
    /// ```
...
impl<'a, T, const N: usize> IntoDistribution<&'a T> for &'a [T; N] {
    type Distribution = Choose<'a, T>;

    type Error = Empty;

    /// Creates a [`Distribution`] which uniformly samples a single element from
    /// a [`&[T; N]`](primitive@array) and returns a reference to it.
    ///
    /// # Example
    /// ```
    /// # use ec_core::distributions::conversion::IntoDistribution;
...
impl<'a, T, const N: usize> IntoDistribution<T> for &'a [T; N]
where
    T: Clone,
{
    type Distribution = ChooseCloning<'a, T>;
...
impl<'a, T> IntoDistribution<&'a T> for &'a [T] {
    ...
    /// Creates a [`Distribution`] which uniformly samples a single element from
    /// a [`&[T]`](primitive@slice) and returns a reference to it.
    ///
    /// # Example
    /// ```
    /// # use ec_core::distributions::conversion::IntoDistribution;
    ...
    ///
    /// let my_value: &i32 = my_distr.sample(&mut rng());
    /// assert!(my_slice.contains(my_value));
    /// # let _ = my_value;
    /// # Ok(())
    /// # }
    /// ```

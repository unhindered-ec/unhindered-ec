...
/// An instruction that check if one value is greater than or equal to another.
///
/// This instruction works similarly to Rust's [`PartialOrd`].
///
/// `GreaterThanEqual<T>` (which
/// is just a shorthand for `GreaterThanEqual<T,T>`) compares the top
...
///
/// If we instead use two different types, like in `GreaterThanEqual<T,U>`, this
...
///
/// The `GreaterThanEqual<T, T>` instruction takes the following inputs:
///    - `T` stack
///      - Two values
///
/// ## `GreaterThanEqual<T, U>`
///
/// The `GreaterThanEqual<T, U>` instruction takes the following inputs:
///    - `T` stack
///       - One value
///    - `U` stack
///       - One value
...
/// # Behavior
///
/// The `GreaterThanEqual` instruction takes top two values from the `T`
...
impl<S, First, Second> Instruction<S> for GreaterThanEqual<First, Second>
where
...
mod test {
    ...
    fn equal() {
        let state = PushState::builder()
            ...
            .with_no_program()
            .build();
        let result = GreaterThanEqual::<i64>::default().perform(state).unwrap();
    ...
    fn empty_stack() {
        let state = PushState::builder()
            ...
            .with_no_program()
            .build();
        let result = GreaterThanEqual::<i64>::default()
            .perform(state)
            .unwrap_err();
